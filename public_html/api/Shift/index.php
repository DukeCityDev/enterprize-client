...
use Unm\Scheduler\{Scon,Pod,ShiftPlan,Shift};
...
try{
    ...
    if($method === "GET"){
        ...
        switch($_GET["findMethod"]){
            case "sconNetIdAndShiftPlan":
                $sconNetId = $_GET["sconNetId"];
                $shiftPlanName = $_GET["shiftPlanName"];
                $shifts = Shift::getShiftsByShiftPlanNameAndSconNetId($pdo,$shiftPlanName,$sconNetId);
                $reply->data = $shifts;
                break;
            case "shiftPlanNameAndPodId":
                $shiftPlanName = $_GET["shiftPlanName"];
                $podId = $_GET["podId"];
        ...
        //Check to make sure the sender is an Administrator
            throw new InvalidArgumentException("Scon is not authorized",401);        $netId = $_SERVER['REDIRECT_REMOTE_USER'];
